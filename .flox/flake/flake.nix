      ...
      default = 
       ...
       arg = {
         ...
         builder = "${builtins.fetchClosure {
           ...
           fromPath = self.packages.${system}.bash-out or (throw "No bash found, install one into the toplevel");
           inputAddressed = true;
         }}/bin/bash";
         inherit system;
         outputs = [ "out" ];
	...
	builtins.fetchClosure {
      ...
      in
      {
        default = builtins.derivation {
          ...
          derivations = builtins.attrValues (builtins.mapAttrs (name: x: [
            "true"
            (toString x.priority)
            1
            (closure x)
      ...
      builtins.mapAttrs (name: pkg: closure pkg) pkgs) self.raw_packages_listing;

    build_packages = builtins.mapAttrs (system: pkgs:
      builtins.mapAttrs (name: pkg: 
        # TODO: Provenance not stored in a usable form in lockfile, assume nixpkgs for now
        let flake = builtins.getFlake "github:flox/nixpkgs/${pkg.rev}";
        in flake.legacyPackages.${system}.${pkg.attr_path} or flake.packages.${system}.${pkg.attr_path}
      ) pkgs) self.raw_packages_listing;

    # Process lockfile into a tree form
    raw_packages_listing = let
      lock = builtins.fromJSON (builtins.readFile ../env/manifest.lock);
      pkgs = lock.packages;
      pkgsBySystem = builtins.groupBy (x: x.system) pkgs;
      pkgsBySystemAttrOutput = builtins.mapAttrs (system: pkgs:
        let
          initGrouping = builtins.concatMap (x: builtins.attrValues (builtins.mapAttrs (k: y: x // {path=y;output=k;}) x.outputs )) pkgs;
          perOutput = builtins.groupBy (x: "${x.attr_path}-${x.output}") initGrouping;
          finalForm =builtins.mapAttrs (k: v:
           let
             pkg = builtins.elemAt v 0;
           in 
             pkg
             ) perOutput;
           in finalForm
